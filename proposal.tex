...
Unfortunately,
the most reliable methods to explore the posterior are based on Markov Chain Monte Carlo (MCMC) algorithms \cite{gelman14}, and
these algorithms require an analytic prior and then output samples from the posterior;
but, how can we reuse samples from the posterior as a new analytical prior?
Every time new data arrive, we are forced to re-train the model,
which is impractical.
This is the core of O.~\ref{prior}.

Some methods to solve this problem are sequential learning or importance sampling.
Recently, NNs have been proposed as a fast
...
\begin{table}[t!]
    ...
    @{}}
        ...
        4 &
        No sufficient funding for expe\-riments,
        they cannot be completed, or
        bad results (\textit{e.g.} malfunctioning sensor). &
        M/H &
        Basic tests are easily within the capabilities of the MTV-RG.
        Also, I could ask a collaborator like IDEKO for historical data, or search public datasets.
        \\ \midrule
...
For computing, personal computers are expected to meet the demand for this project, but
even if they do not,
...
IDEKO.
In Hungary, Z. Dombóvári also collaborates with researchers from Bosch, which
offers access to additional equipment and data.

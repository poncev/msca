...
Unfortunately,
technical issues make it difficult to realize this philosophy in practice, and
the model must be re-trained from scratch with the new dataset.
This makes, in particular, Bayesian inference impractical in structures with varying parameters.
This is the content of O.~\ref{prior}.
...
it is impossible to reuse a previous posterior which is a collection of samples, not an analytic function.

Several alternatives have been given to solve this problem,
...
For instance, a method called Normalizing Flow was used to convert samples into an analytic prior \(p(M)\) in a problem of reconstruction of images from partial or corrupted data \cite{cai24}.

The last strategic objective (\obj{experiment}) is
\emph{to validate a methodology experimentally.}
I must select at least one structure with nonlinear behavior, and
as a starting point I can use the datasets in \cite{schoukens17}.
